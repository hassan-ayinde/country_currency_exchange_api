...
const { sequelize, Country, Meta } = require('../models/index');
  ...
  try {
    ...
    const processed = countries.map((c) => {
      ...
      return {
        ...
        estimated_gdp, flag_url, last_refreshed_at: now
      };
    });
    ...
    for (const p of processed) {
      const existing = await Country.findOne({
        where: sequelize.where(sequelize.fn('lower', sequelize.col('name')), sequelize.fn('lower', p.name)),
        transaction: t
      });
      if (existing) {
        ...
        await Country.create(p, { transaction: t });
      }
    }

    // update meta last_refreshed_at
    await Meta.upsert({ key: 'last_refreshed_at', value: new Date().toISOString() }, { transaction: t });
    ...
    await generateSummaryImage({ total, top5, timestamp: new Date().toISOString() });
